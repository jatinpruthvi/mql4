...
extern string CustomComment = "Strategy 5.14";
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 5.14", 9, "Tahoma", LabelColor);
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(232 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(232 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = Close[11] + (-0.5) * (iATR(NULL, 0, 6, 1));
