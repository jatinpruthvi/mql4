...
extern string CustomComment = "Strategy 8.10";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      if(TradeLong) {
         bool LongEntryCondition = ruleLongTermStoch_Below();
      ...
      if(TradeShort) {
         bool ShortEntryCondition = ruleLongTermStoch_Above();
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 8.10", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(getHighest(77, 1) + (0.1) * (gbBarRange(33, 1)));
   } else {
      // short
      return(getLowest(77, 1) + (-0.1) * (gbBarRange(33, 1)));
   }
}
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT((0.9 * iATR(NULL, 0, 57, 1))));
   } else {
      // short
      return(checkCorrectMinMaxSLPT((0.9 * iATR(NULL, 0, 57, 1))));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(298 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(298 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = iMA(NULL, 0, 77, 0, MODE_EMA, PRICE_CLOSE, 1) + (0.1) * (iCustom(NULL, 0, "BBandWidthRatio", 86, 2.0, 0, 1));
