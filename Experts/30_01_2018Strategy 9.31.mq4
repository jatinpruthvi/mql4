...
extern string CustomComment = "Strategy 9.31";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: Close Below Bollinger Bands(20, 2, 0)
      if(TradeLong) {
         bool LongEntryCondition = ruleCloseBelowBB();
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: Close Above Bollinger Bands(20, 2, 0)
      if(TradeShort) {
         bool ShortEntryCondition = ruleCloseAboveBB();
   ...
   if(getMarketPosition() != 0) {
      ...
      if(isMarketLongPosition()) {
         ...
         if (getOpenBarsForOrder(214) >= 214) {
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 9.31", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(iIchimoku(NULL, 0, 6, 18, 38, MODE_KIJUNSEN, 1) + (0.6) * (iCustom(NULL, 0, "BBandWidthRatio", 17, 2.0, 0, 1)));
   } else {
      // short
      return(iIchimoku(NULL, 0, 6, 18, 38, MODE_KIJUNSEN, 1) + (-0.6) * (iCustom(NULL, 0, "BBandWidthRatio", 17, 2.0, 0, 1)));
   }
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(295 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(295 * gPointCoef));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(226 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(226 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = iMA(NULL, 0, 56, 0, MODE_EMA, PRICE_CLOSE, 1) + (0) * (iCustom(NULL, 0, "BBandWidthRatio", 17, 2.0, 0, 1));
