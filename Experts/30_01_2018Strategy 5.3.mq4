...
extern string CustomComment = "Strategy 5.3";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: Short Term RSI(20) > 50
      if(TradeLong) {
         bool LongEntryCondition = ruleShortTermRSI_Above();
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: Short Term RSI(20) < 50
      if(TradeShort) {
         bool ShortEntryCondition = ruleShortTermRSI_Below();
   ...
   if(getMarketPosition() != 0) {
      ...
      if(isMarketLongPosition()) {
         ...
         if (getOpenBarsForOrder(184) >= 184) {
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 5.3", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(iCustom(NULL, 0, "LinReg", 30, 0, 1) + (0.5) * (iCustom(NULL, 0, "BBandWidthRatio", 77, 3.6, 0, 1)));
   } else {
      // short
      return(iCustom(NULL, 0, "LinReg", 30, 0, 1) + (-0.5) * (iCustom(NULL, 0, "BBandWidthRatio", 77, 3.6, 0, 1)));
   }
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT((1.5 * iATR(NULL, 0, 51, 1))));
   } else {
      // short
      return(checkCorrectMinMaxSLPT((1.5 * iATR(NULL, 0, 51, 1))));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(120 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(120 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = Low[11] + (0.9) * (iATR(NULL, 0, 7, 1));

   } else if (OrderType() == OP_SELL) {
      value = High[11] + (-0.9) * (iATR(NULL, 0, 7, 1));
   }
