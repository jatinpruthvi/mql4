...
//|    Generated at Sat Sep 06 21:02:00 GMT 2014
//|
...
extern string CustomComment = "Strategy 1.27";
...
int start() {
   ...
   // ENTRY RULES
      ...
      // LONG: Long Term Stochastic(40, 1, 3) > 50
      if(TradeLong) {
         bool LongEntryCondition = ruleLongTermStoch_Above();
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: Long Term Stochastic(40, 1, 3) < 50
      if(TradeShort) {
         bool ShortEntryCondition = ruleLongTermStoch_Below();
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 1.27", 9, "Tahoma", LabelColor);
      ...
      ObjectSetText("linec", "Generated by StrategyQuant 3.6.3", 8, "Tahoma", LabelColor);
...
void manageTradeSLPT() {
   ...
   double spread = Ask-Bid;
  	double bidLow = Low[1];
  	double bidHigh = High[1];
  	double askLow = Low[1]+spread;
  	double askHigh = High[1]+spread;
   ...
   for(int i=0; i<OrdersTotal(); i++) {
      if (OrderSelect(i,SELECT_BY_POS)==true && OrderMagicNumber() == MagicNumber && OrderSymbol() == Symbol()) {
         ...
         SL = getNormalSL(OrderStopLoss());
         PT = getNormalPT(OrderTakeProfit());
...
double getAsk() {
   return(Low[1]+gbSpread*gPointCoef);
}
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(HeikenAshiClose(1) + (0.1) * (gbBarRange(51, 1)));
   } else {
      // short
      return(HeikenAshiClose(1) + (-0.1) * (gbBarRange(51, 1)));
   }
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(44 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(44 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = High[10] + (0.8) * (iATR(NULL, 0, 30, 1));

   } else if (OrderType() == OP_SELL) {
      value = Low[10] + (-0.8) * (iATR(NULL, 0, 30, 1));
   }
