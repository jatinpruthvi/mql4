...
extern string CustomComment = "Strategy 28.25";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: (ATR(15) < ATR(39))
      if(TradeLong) {
         bool LongEntryCondition = (iATR(NULL, 0, 15, 1) < iATR(NULL, 0, 39, 1));
      ...
      // SHORT: (ATR(15) > ATR(39))
      if(TradeShort) {
         bool ShortEntryCondition = (iATR(NULL, 0, 15, 1) > iATR(NULL, 0, 39, 1));
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 28.25", 9, "Tahoma", LabelColor);
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT((3.94 * iATR(NULL, 0, 39, 1))));
   } else {
      // short
      return(checkCorrectMinMaxSLPT((3.94 * iATR(NULL, 0, 39, 1))));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT((3 * iATR(NULL, 0, 39, 1))));
   } else {
      // short
      return(checkCorrectMinMaxSLPT((3 * iATR(NULL, 0, 39, 1))));
   }
