...
extern string CustomComment = "Strategy 7.25";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      if(TradeLong) {
         bool LongEntryCondition = ruleShortTermRSI_Below();
      ...
      if(TradeShort) {
         bool ShortEntryCondition = ruleShortTermRSI_Above();
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 7.25", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(getHighest(87, 1) + (-0.2) * (iATR(NULL, 0, 27, 1)));
   } else {
      // short
      return(getLowest(87, 1) + (0.2) * (iATR(NULL, 0, 27, 1)));
   }
}
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT((0.79 * iATR(NULL, 0, 4, 1))));
   } else {
      // short
      return(checkCorrectMinMaxSLPT((0.79 * iATR(NULL, 0, 4, 1))));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(185 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(185 * gPointCoef));
   }
