...
extern string CustomComment = "Strategy 8.16";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: Long Term CCI(40) < 0
      if(TradeLong) {
         bool LongEntryCondition = ruleLongTermCCI_Below();
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: Long Term CCI(40) > 0
      if(TradeShort) {
         bool ShortEntryCondition = ruleLongTermCCI_Above();
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 8.16", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(getHighest(89, 1) + (0.4) * (gbTrueRange(89, 1)));
   } else {
      // short
      return(getLowest(89, 1) + (-0.4) * (gbTrueRange(89, 1)));
   }
}
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(98 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(98 * gPointCoef));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(238 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(238 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = iIchimoku(NULL, 0, 9, 26, 52, MODE_SENKOUSPANB, 1) + (0.5) * (iCustom(NULL, 0, "BBandWidthRatio", 2, 2.0, 0, 1));

   } else if (OrderType() == OP_SELL) {
      value = iIchimoku(NULL, 0, 9, 26, 52, MODE_SENKOUSPANA, 1) + (-0.5) * (iCustom(NULL, 0, "BBandWidthRatio", 2, 2.0, 0, 1));
   }
