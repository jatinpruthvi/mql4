...
extern string CustomComment = "Strategy 5.13";
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 5.13", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(getHighest(90, 1) + (0) * (gbBarRange(11, 1)));
   } else {
      // short
      return(getLowest(90, 1) + (0) * (gbBarRange(11, 1)));
   }
}
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(270 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(270 * gPointCoef));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(278 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(278 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = High[17] + (0.2) * (gbBarRange(82, 1));

   } else if (OrderType() == OP_SELL) {
      value = Low[17] + (-0.2) * (gbBarRange(82, 1));
   }
