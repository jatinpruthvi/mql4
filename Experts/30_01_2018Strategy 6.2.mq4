...
extern string CustomComment = "Strategy 6.2";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: (Williams%R(97) > MACD Signal(12, 26, 9))
      if(TradeLong) {
         bool LongEntryCondition = (iWPR(NULL, 0, 97, 1) > iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, 1));
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: (Williams%R(97) < MACD Signal(12, 26, 9))
      if(TradeShort) {
         bool ShortEntryCondition = (iWPR(NULL, 0, 97, 1) < iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, 1));
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 6.2", 9, "Tahoma", LabelColor);
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = iIchimoku(NULL, 0, 9, 26, 52, MODE_SENKOUSPANA, 1) + (0.2) * (iATR(NULL, 0, 89, 1));

   } else if (OrderType() == OP_SELL) {
      value = iIchimoku(NULL, 0, 9, 26, 52, MODE_SENKOUSPANB, 1) + (-0.2) * (iATR(NULL, 0, 89, 1));
   }
