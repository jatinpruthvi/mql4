...
extern string CustomComment = "Strategy 15.359";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: (RSI(29) < 41.450001)
      if(TradeLong) {
         bool LongEntryCondition = (iRSI(NULL, 0, 29, PRICE_CLOSE, 1) < 41.450001);
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: (RSI(29) > 58.549999)
      if(TradeShort) {
         bool ShortEntryCondition = (iRSI(NULL, 0, 29, PRICE_CLOSE, 1) > 58.549999);
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 15.359", 9, "Tahoma", LabelColor);
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(249 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(249 * gPointCoef));
   }
