...
extern string CustomComment = "Strategy 8.9";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: Long Term RSI(40) < 50
      if(TradeLong) {
         bool LongEntryCondition = ruleLongTermRSI_Below();
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: Long Term RSI(40) > 50
      if(TradeShort) {
         bool ShortEntryCondition = ruleLongTermRSI_Above();
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 8.9", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(getHighest(98, 1) + (0) * (gbBarRange(30, 1)));
   } else {
      // short
      return(getLowest(98, 1) + (0) * (gbBarRange(30, 1)));
   }
}
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(91 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(91 * gPointCoef));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(170 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(170 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = HeikenAshiLow(6) + (-0.5) * (iATR(NULL, 0, 6, 1));

   } else if (OrderType() == OP_SELL) {
      value = HeikenAshiHigh(6) + (0.5) * (iATR(NULL, 0, 6, 1));
   }
