...
extern string CustomComment = "Strategy 8.20";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: (Williams%R(38) > MACD Signal(12, 26, 9))
      if(TradeLong) {
         bool LongEntryCondition = (iWPR(NULL, 0, 38, 1) > iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, 1));
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: (Williams%R(38) < MACD Signal(12, 26, 9))
      if(TradeShort) {
         bool ShortEntryCondition = (iWPR(NULL, 0, 38, 1) < iMACD(NULL, 0, 12, 26, 9, PRICE_CLOSE, MODE_SIGNAL, 1));
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 8.20", 9, "Tahoma", LabelColor);
...
double getTradeOpenPrice(int tradeDirection) {
   ...
   if(tradeDirection == 1) {
      // long
         return(iIchimoku(NULL, 0, 9, 26, 52, MODE_KIJUNSEN, 1) + (0.7) * (iCustom(NULL, 0, "BBandWidthRatio", 68, 2.0, 0, 1)));
   } else {
      // short
      return(iIchimoku(NULL, 0, 9, 26, 52, MODE_KIJUNSEN, 1) + (-0.7) * (iCustom(NULL, 0, "BBandWidthRatio", 68, 2.0, 0, 1)));
   }
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(87 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(87 * gPointCoef));
   }
...
double getProfitTarget(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(216 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(216 * gPointCoef));
   }
...
double getStopTrailingByClose() {
   double value = 0;
   if (OrderType() == OP_BUY) {
      value = iIchimoku(NULL, 0, 12, 26, 9, MODE_SENKOUSPANA, 1) + (0.2) * (iATR(NULL, 0, 89, 1));

   } else if (OrderType() == OP_SELL) {
      value = iIchimoku(NULL, 0, 12, 26, 9, MODE_SENKOUSPANB, 1) + (-0.2) * (iATR(NULL, 0, 89, 1));
   }
