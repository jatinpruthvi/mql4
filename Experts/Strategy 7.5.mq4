...
//|    Generated at Thu Jan 25 14:30:00 GMT 2018
//|                                                          
//|    Tested on EURJPY_dks, M5, 02.01.2015 - 31.01.2017
...
extern string CustomComment = "Strategy 7.5";
...
int start() {
   ...
   if(manageTrades()) {
      ...
      // LONG: ((18 + CCI(63)) < RSI(63))
      if(TradeLong) {
         bool LongEntryCondition = ((18 + iCCI(NULL, 0, 63, PRICE_TYPICAL, 1)) < iRSI(NULL, 0, 63, PRICE_CLOSE, 1));
         if(LongEntryCondition == true) {
            openPosition(1);
         }
      }
   
      // SHORT: ((18 + CCI(63)) > RSI(63))
      if(TradeShort) {
         bool ShortEntryCondition = ((18 + iCCI(NULL, 0, 63, PRICE_TYPICAL, 1)) > iRSI(NULL, 0, 63, PRICE_CLOSE, 1));
...
int init() {
   ...
   if(DisplayInfoPanel) {
      ...
      ObjectSetText("line1", "Strategy 7.5", 9, "Tahoma", LabelColor);
...
double getStopLoss(int tradeDirection) {
   if(tradeDirection == 1) {
      // long
      return(checkCorrectMinMaxSLPT(263 * gPointCoef));
   } else {
      // short
      return(checkCorrectMinMaxSLPT(263 * gPointCoef));
   }
